...
## Repostory에 올리는 법
### 최초일 때
1. 깃허브 레포지토리 만들기! 새 파일이면 새 레포지토리를 만들어야 한다.
2. 바탕화면에 폴더 만들기
3. VSCode에서 폴더 열고 올릴 파일 만들기
   - 아무 파일이나 우클릭해서 Reveal in Finder 눌러서 폴더 안의 `.git` 폴더를 삭제해줘야 한다!
4. 저장소 초기화
...
8. 깃허브에 올리기 push(레포지토리 위에 줄 복사해서 붙여야 되는데 7.번에 포함되어 있어서 안해도 됨)
  ...
  [개인 심화과제 3번까지 구현 / D251212.dart파일] 터미널에서 
  - dart run code/D파일명.dart 누르면 입력창이 떠서 ex) 학생이름 검색하면 점수와 정보가 같이 뜬다.
...
- TODO 주석 작성할 때 `//` 주석 바로 뒤에 작성해야 TODO tree에 나옴

이렇게 하면 Git 마크다운 폴더에 이미지가 저장된다!!! 
저장 후 확인: 우클릭- 마크다운 프리뷰

- 모든 파일은 하드디스크(보조기억장치)에 저장, RAM(주기억장치)실행할 때 저장이 됨 실행하기 위한 것, cpu는 연산 
  변수를 선언하면 주기억장치 어딘가 저장이 됨. 긴 리스트를 사용할 때 사용하는 공간이 늘어난다 이걸 줄여야 함
  사용되는 메모리를 적게 하는게 중요
- 공간복잡도: 반복문이 아주 길어질 때 0(n) 데이터를 얼마나 복잡하게 저장하느냐
  -> 계산이 완료되는데 까지 필요한 공간(메모리)가 얼마나 되느냐(변수를 몇개나 선언했냐)
- 시간복잡도: 0(n^2) . 정렬과 시간복잡도에 대해 알아보기
  -> 계산이 완료되는데 까지 필요한 시간이 얼마나 걸리냐(반복문을 얼마나 사용했나)
  3차원의 좌표를 저장해야 된다고 했을 때, 3차원 배열은 List<List<List<int>>> list3d = [];
  int 하나의 공간이 n의 3제곱 만큼 필요한 것. n^3
  https://rnflajdrnfl.tistory.com/18 - 참고 정리 내용
...
- PPT도 마크다운 방식으로 제작 가능하다.
  위에서 > reveal : Export in HTML 누르기
  revealjs presentation 

  - 펑션 누르면 아이콘이 뜸!!!!!!!😙다형성 (펑션키 누르면 아이콘 나옴. 일반키보드는 커맨드+컨트롤+스페이스바)
  - 파일명 소문자로 시작해야 되는 것은 아니고, 프로젝트에 따라 다름